...
def main():
  ...
  nObjects = 3
  nSurfaceObjects = 3
  objectFolder = os.getcwd() + "/../../Data/RaveObjects/Bottles"
  surfObjectFolder = os.getcwd() + "/../../Data/RaveObjects/Coasters"
  graspColor = [0,0,1]
  placeColor = [1,0,0]

  # reaching
  nActionSamples = [500, 1000, 361, 181, 361, 303]
  stateImageWHD = [0.09, 0.20, 0.09]
  ...
  for i in xrange(nAgents):
    rlAgent = eval("RlAgentLevel" + str(i) + \
      "(rlEnv, gpuId, False, nActionSamples[i], \"weights-grasp-bottles/\")")
  ...
  for i in xrange(nAgents):
    rlAgent = eval("RlAgentLevel" + str(i) +
      "(rlEnv, gpuId, True, nActionSamples[i], \"weights-place-bottles/\")")
  ...
  for trainingRound in xrange(nTrainingRounds):
    ...
    for scene in xrange(nScenes):
      ...
      objHandles = rlEnv.PlaceObjects(
        randint(1, nObjects+1), objectFolder, activeOrientOpts=[0,1,2,3,4])
      surfObjHandles = rlEnv.PlaceObjects(
        nSurfaceObjects, surfObjectFolder, activeOrientOpts=[4])
      ...
      for episode in xrange(nEpisodes):
        ...
        if rGrasp > 0.00:
          ...
          rPlace = rlEnv.TransitionPlaceUprightOnSurfaceObject(
            placeDesc, rlAgentsPlacing[-1], objHandles, surfObjHandles, showSteps)
      ...
      rlEnv.RemoveObjectSet(surfObjHandles)
    ...
    saveData = {"gpuId":gpuId, "nAgents":nAgents, "nObjects":nObjects, "objectFolder":objectFolder,
      "surfObjectFolder":surfObjectFolder, "viewCenter":viewCenter, "viewKeepout":viewKeepout,
      "viewWorkspace":viewWorkspace, "viewWorkspaceNoTable":viewWorkspaceNoTable,
      "nActionSamples":nActionSamples, "stateImageWHD":stateImageWHD,
      "nTrainingRounds":nTrainingRounds, "nScenes":nScenes, "nEpisodes":nEpisodes,
      "epsilonGraspRound":epsilonGraspRound, "epsilonPlaceRound":epsilonPlaceRound,
      "minEpsilon":minEpsilon, "unbiasOnRound":unbiasOnRound, "maxExperiences":maxExperiences,
      "trainingBatchSize":trainingBatchSize, "minTrainBatchSize":minTrainBatchSize,
      "avgReturn":avgReturn, "avgPlaceReward":avgPlaceReward, "avgGraspReward":avgGraspReward,
      "graspDatabaseSize":graspDatabaseSize, "placeDatabaseSize":placeDatabaseSize,
      "roundTime":roundTime,
